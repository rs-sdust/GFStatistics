    ...
    public static class FtpClient
    {
        ...
        public static bool FtpBrokenDownload(string remoteFileName, string localFileName, bool ifCredential, long size, Action<int, int> updateProgress = null)
        {
            ...
            try
            {
                ...
                if (updateProgress != null)
                {
                    updateProgress((int)totalBytes,(int)size);//更新进度条     
                }
        ...
        /// 删除文件
        /// </summary>
        /// <param name="fileName">ftp相对地址</param>
        public static bool DeleteFileName(string remoteFileName,bool ifCredential = false)
        {
            FtpWebResponse response = null;
            bool sucess = true;
            try
        ...
        public static bool MakeDir(string dirName, bool ifCredential = false)
        {
            FtpWebResponse response = null;
            ...
            try
            {
                ...
                Uri uri = new Uri("ftp://" + FtpServerIP + "/" + dirName);
                reqFTP = (FtpWebRequest)FtpWebRequest.Create(uri);
                reqFTP.UseBinary = true;
                reqFTP.KeepAlive = false;
                if (ifCredential)//使用用户身份认证  
